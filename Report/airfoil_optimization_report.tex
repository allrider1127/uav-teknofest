...
\begin{abstract}
This paper presents a multi-point aerodynamic shape optimization of a 3D wing airfoil designed for a 16.5 kg fixed-wing Unmanned Aerial Vehicle (UAV) participating in the TEKNOFEST Advanced Autonomous Systems Competition. The design requires a high maximum lift coefficient to reduce stall speed and catapult launch energy, combined with high aerodynamic efficiency in both cruise and high-G turning flight to maximize the flight performance score. To guarantee a smooth, manufacturable shape free of geometric waviness, a 4th-order Class Shape Transformation (CST) parameterization method is employed. The optimization is conducted using a parallelized Sequential Least Squares Programming (SLSQP) algorithm coupled with XFOIL to evaluate viscous polars at $Re = 544,000$. The robust optimization successfully balances cruise drag reduction, high-lift turn capability, and structural thickness constraints. The optimized airfoil shows a 3.4\% increase in maximum lift and a 20.1\% reduction in pitching moment, ensuring a highly stable, efficient, and structurally compliant wing design.
\end{abstract}
...
In low-Reynolds-number aerodynamics ($Re \approx 300,000 - 600,000$), the flow is highly sensitive to laminar separation bubbles, which can cause premature stall or catastrophic drag increases. To address this, we present an automated, parallelized multi-point optimization system. We build on the turn radius minimization work of Koroglu and Ozkol (Istanbul Turn Radius, 2019) and the Class Shape Transformation (CST) method of Kulfan (Boeing, 2008) to generate a mathematically smooth, highly efficient, and structurally robust airfoil shape.
...
XFOIL combines a 2D panel method with a two-equation integral boundary layer solver to resolve the viscous-inviscid flow field. The governing physics solved by XFOIL's numerical engine along the surface arc length coordinate $s$ are detailed below:
...
\begin{equation}
    \text{Minimize } J(\mathbf{w}) = 0.60 \cdot C_d(C_l=0.65) + 0.40 \cdot C_d(C_l=1.20) + \text{Penalties}
\end{equation}
...
\centering
\small
\caption{Airfoil Optimization Performance Comparison}
\label{tab:performance}
\begin{tabular}{lccc}
\toprule
Parameter & Baseline (SD7062) & Multi-Point & Optimized (Perfect) \\
\midrule
Max Thickness ($t/c$) & 14.02\% & 13.84\% & 14.22\% \\
LE Radius ($r_{\text{le}}$) & 8.18\% & 7.15\% & 7.45\% \\
$C_{l,max}$ (Stall) & 1.6925 & 1.5301 & 1.5873 \\
Cruise $C_d$ ($C_l=0.65$) & 0.00814 & 0.00765 & 0.00781 \\
Turning $C_d$ ($C_l=1.20$) & 0.01151 & 0.01050 & 0.01058 \\
Cruise $C_m$ ($C_l=0.65$) & -0.08230 & -0.06726 & -0.07374 \\
Geometric Quality & Smooth & Reflexed / High-Performance & Perfectly Rounded LE \\
\bottomrule
\end{tabular}
\end{table}
...
The geometric and polar comparison plots are presented in Fig.~\ref{fig:geometry} and Fig.~\ref{fig:polars}, respectively.
...
\begin{itemize}
    \item An \textbf{8.1\% reduction in turning drag} ($C_d = 0.01058$), allowing the UAV to maintain energy and speed during autonomous turns, reducing $d_{\text{UAV}}$ to increase the Duration Score.
    \item A \textbf{4.1\% reduction in cruise drag} ($C_d = 0.00781$), lowering power consumption and allowing a smaller battery pack, which directly reduces $w_{\text{UAV}}$ to increase the Weight Score.
    \item A \textbf{10.4\% reduction in pitching moment} ($C_m = -0.07374$), reducing trim drag.
    \item A maximum lift coefficient $C_{l,max} = 1.5873$, yielding an actual stall speed $V_s = 13.5\text{ m/s}$ (comfortably below the $15.2\text{ m/s}$ limit), ensuring safe catapult launch and parachute landing.
\end{itemize}
...
\midrule
Upper ($w_{\text{up}}$) & 0.25155 & 0.31360 & 0.30740 & 0.23237 & 0.16449 \\
Lower ($w_{\text{low}}$) & -0.13509 & -0.06364 & -0.07558 & -0.03586 & 0.01985 \\
\bottomrule
\end{tabular}
\end{table}
...
0.05 & 0.05583 & -0.02589 \\
0.10 & 0.07751 & -0.03160 \\
0.20 & 0.10196 & -0.03362 \\
0.30 & 0.11105 & -0.03111 \\
0.40 & 0.10893 & -0.02677 \\
0.50 & 0.09820 & -0.02136 \\
0.60 & 0.08136 & -0.01527 \\
0.70 & 0.06082 & -0.00903 \\
0.80 & 0.03898 & -0.00353 \\
0.90 & 0.01816 & -0.00010 \\
0.95 & 0.00881 & 0.00031 \\
